...
from OCC.Core.STEPControl import (STEPControl_AsIs, STEPControl_Writer)
from OCC.Core.STEPConstruct import stepconstruct # _FindEntity
...
def STEPwriter(elements, data, write_file):
    
    if not write_file.endswith("step"): 
       write_file = f"{write_file}.step"
...
#   schema = 'AP203'
    ...
    for element in selected_elements:
        if element.Representation is not None:
            ...
            Interface_Static.SetCVal('write.step.product.name', eleName)
    ...
    print("Writing")
    writer.Write(write_file), read_step_file_with_names_colors(write_file)
